...
class SB_Backlinks_List_Table extends WP_List_Table
{
    ...
    protected function column_default( $item, $column_name )
    {
        ...
        switch ( $column_name ) {
            case 'ref':
                return '<a href="' . $item['ref'] . '" target="_blank">' . $seobooster2->truncatestring( $seobooster2->remove_http( $item['ref'] ), 35 ) . '</a>';
            case 'httpstatus':
                return $item[$column_name];
            case 'lp':
                return $seobooster2->truncatestring( $seobooster2->remove_http( $item['lp'] ), 35 );
            case 'visits':
                return $item[$column_name];
            case 'firstvisit':
                return $item[$column_name];
