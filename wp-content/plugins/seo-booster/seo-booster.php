...
Copyright 2008-2017 cleverplugins.com
...
TODO - transient cache for keyword replacement
...
define( 'SEOBOOSTER_VERSION', '3.3.1' );
...
function seobooster_fs_custom_connect_message_on_update(
    $message,
    $user_first_name,
    $plugin_title,
    $user_login,
...
{
    return sprintf(
        __( 'Hey %1$s' ) . ',<br>' . __( 'Please help us improve %2$s! If you opt-in, some data about your usage of %2$s will be sent to %5$s. If you skip this, that\'s okay! %2$s will still work just fine.', 'seo-booster' ),
        $user_first_name,
        '<b>' . $plugin_title . '</b>',
        '<b>' . $user_login . '</b>',
        $site_link,
...
seobooster_fs()->add_filter(
    'connect_message_on_update',
    'seobooster_fs_custom_connect_message_on_update',
    10,
    6
);
...
if ( !class_exists( "seobooster2" ) ) {
    class seobooster2
    {
        ...
        function __construct()
        {
            ...
            add_action( 'plugins_loaded', array( &$this, 'do_load_plugin_textdomain' ) );
            ...
            /*
            // Removed for now, can be reintroduced when reworked later
            add_action('admin_bar_menu', array(&$this, 'admin_bar_menu_add_details_dropdown'),2000);
            */
        ...
        function containsWord( $str, $word )
        {
            ...
            return $matches;
            //return !!preg_match('#\\b' . preg_quote($word, '#') . '\\b#i', $str, $matches);
        }
        
        function do_filter_the_content( $content )
        {
            ...
            if ( is_singular() && in_the_loop() ) {
                ...
                // todo - get the custom keywords from SBP also.
                ...
                if ( $use_yoast_focus_kw ) {
                    ...
                    // TODO
                    // Convert keywords to array with urls
                ...
                if ( isset( $searchReplaceArr ) && !$searchReplaceArr && get_option( 'seobooster_debug_logging' ) == 'on' ) {
                    $this->log( __( 'Auto link turned on, but no keywords in system ', 'seo-booster' ) . ' ' . $currenturl );
                    //return $content;
                }
                
                //error_log('replacing '.count($searchReplaceArr).' from '.$currenturl); // SSSS TODO
                // SSSS - todo - replace array - gør unik
                ...
                foreach ( $html->find( 'text' ) as $text ) {
                    ...
                    foreach ( $searchReplaceArr as $key => $sr ) {
                        ...
                        if ( !in_array( $sr['kw'], $ReplaceKWArr ) && $replaceCount < $replaceLimit ) {
                            $stepCount++;
                            $totalStepCount++;
        ...
        function do_load_plugin_textdomain()
        {
        ...
        // Runs on action "template_redirect"
        ...
        function plugins_loaded_register_visitor()
        {
            ...
            if ( isset( $_SERVER['HTTP_REFERER'] ) ) {
                ...
                if ( $this->ignorelink( $referer ) ) {
                    ...
                    // 	$this->log(sprintf(__("Debug: Ignoring referrer <code>%s</code>", 'seo-booster'), $referer));
                    // }
                    return;
                }
            }
        ...
        function send_email_update( $days = 7, $forced = false )
        {
            ...
            if ( $knownkws ) {
                ...
                foreach ( $knownkws as $keyword ) {
                    ...
                    $content .= '<p><a href="' . $keyword->lp . '">' . $this->remove_http( $keyword->lp ) . '</a></br>';
                    ...
                    if ( $kws_comma_separated != '' ) {
                        $content .= '<p>' . __( 'New Keywords:', 'seo-booster' ) . ' ' . $kws_comma_separated . '</p>';
                    }
                }
        ...
        function add_settings_link( $actions, $plugin_file )
        {
            ...
            if ( $plugin == $plugin_file ) {
                ...
                $documentation = array(
                    'documentation' => '<a href="http://cleverplugins.helpscoutdocs.com/" target="_blank">' . __( 'Knowledgebase', 'General' ) . '</a>',
                );
        ...
        function fn_my_ajaxified_dataloader_ajax()
        {
            ...
            if ( isset( $_GET['showkws'] ) && $_GET['showkws'] == 'unknowns' ) {
            ...
            if ( isset( $_GET['showkws'] ) && $_GET['showkws'] == 'knowns' ) {
            ...
            $filteredresults = "SELECT count(kw) FROM {$kwtable} {$sWhere} ";
            $iFilteredTotal = $wpdb->get_var( $filteredresults );
            $sQuery = "\n\t\t\tSELECT FOUND_ROWS()\n\t\t\t";
            $rResultFilterTotal = $wpdb->get_results( $sQuery, ARRAY_N );
            $rResultFilterTotal = $wpdb->num_rows;
            // $sQuery = "
                ...
                for ( $i = 0 ;  $i < count( $aColumns ) ;  $i++ ) {
                    ...
                    if ( $aColumns[$i] == "version" ) {
                        ...
                        if ( $aColumns[$i] == 'lp' ) {
                            ...
                            // Looks up the url and checks if in 404 table
                            $extraclasses = '';
                            // TODO - speed this up
                            /*
                            if ($trimmed) {
                            	$trquery = "SELECT COUNT(*) as cnt FROM {$wpdb->prefix}sb2_404 where lp = '".$trimmed."';";
                            	if ($wpdb->get_var($trquery) > 0 ) {
                            		$extraclasses=" class='foferrorlp'";
                            	}
                            }
                            */
                                ...
                                if ( $aColumns[$i] == 'lastvisit' ) {
                                    ...
                                    if ( $aColumns[$i] == 'engine' ) {
                                        ...
                                        if ( $aColumns[$i] == 'kw' ) {
                                            ...
                                            if ( $aRow[$aColumns[$i]] == '#' ) {
                                                $aRow[$aColumns[$i]] = '<span class="unknown">' . __( 'Not Available', 'seo-booster' ) . '</span>';
                                                // setting unknown class
                                                // todo output different class + <unknown>
                                            } else {
                                            }
                                            
                                            $row[] = $aRow[$aColumns[$i]];
                                            // Output for keywords in table
                                        } else {
                                            if ( $aColumns[$i] != ' ' ) {
                                                $row[] = $aRow[$aColumns[$i]];
                                            }
        ...
        function admin_enqueue_scripts()
        {
            ...
            if ( is_object( $screen ) && $screen->id == 'toplevel_page_sb2_dashboard' or $screen->id == 'seo-booster_page_sb2_log' or $screen->id == 'seo-booster_page_sb2_settings' or $screen->id == 'seo-booster_page_sb2_404' or $screen->id == 'seo-booster_page_sb2_crawled' or $screen->id == 'seo-booster_page_sb2_forgotten' or $screen->id == 'seo-booster_page_sb2_keywords' or $screen->id == 'seo-booster_page_sb2_backlinks' or $screen->id == 'seo-sb2_dashboard' or $screen->id == 'dashboard' ) {
                ...
                wp_enqueue_script( 'lazyload', plugins_url( '/js/jquery.lazyload.min.js', __FILE__ ) );
                wp_enqueue_script( 'seoboosterjs', plugins_url( '/js/seo-booster.js', __FILE__ ) );
                wp_enqueue_style( 'seoboostercss', plugins_url( '/css/seo-booster-min.css', __FILE__ ) );
            }
            ...
            if ( is_object( $screen ) && $screen->id == 'seo-booster_page_sb2_keywords' ) {
                ...
                wp_register_script( 'dataTable', plugins_url( '/js/datatable.js', __FILE__ ) );
        ...
        function list_keywords( $limit = 10, $currurl = '' )
        {
            ...
            $kwtable = $wpdb->prefix . "sb2_kw";
            $ignorelist = get_option( 'seobooster_ignorelist' );
            $ignorelistsize = sizeof( $ignorelist );
            $parsed = preg_replace( "/\r|\n/", ',', $ignorelist );
            $parsed = preg_replace( '/,+/', ',', $parsed );
        ...
        function do_admin_notices()
        {
            ...
            if ( isset( $_GET['sb_message'] ) && !empty($_GET['sb_message']) ) {
                $message = urldecode( $_GET['sb_message'] );
                ?>
		<div class="error">
			<p><?php 
                echo  $message ;
        ...
        function admin_init()
        {
            register_setting( 'seobooster_license', 'seobooster_license_key' );
        ...
        {
            include_once 'seo-booster-seobooster2.php';
        }
        
        function add_seobooster2_settings()
        {
            include_once 'seo-booster-settings.php';
        }
        
        function add_seobooster2_kwpage()
        {
            include_once 'seo-booster-keywords.php';
        }
        
        function add_seobooster2_blpage()
        {
            include_once 'seo-booster-backlinks.php';
        }
        
        function add_seobooster2_forgottenpage()
        {
            include_once 'seo-booster-forgotten.php';
        }
        
        function add_seobooster2_404page()
        {
            include_once 'seo-booster-404s.php';
        }
        
        function add_seobooster2_logpage()
        {
            include_once 'seo-booster-log.php';
        }
	...
	<?php 
                ...
                if ( $strpos !== FALSE ) {
                    ...
                    if ( isset( $se['q'] ) ) {
                        ...
                        $returnarr = array(
                            ...
                            'Se'           => $url_info['host'],
                            'Query'        => strtolower( $query_info[$query_field_in_use] ),
                            'Referstring'  => $inurl,
                        );
                        return $returnarr;
                    }
        ...
        // Returns the current url either by detecting current url from WP or PHP, returns sanitized, unencoded url.
        function seobooster_currenturl( $full = false )
        {
            ...
            global  $wp ;
            //$wpdetected = trailingslashit(strtok(add_query_arg($wp->query_string, '', $wp->request), "?"));
            
            if ( $_SERVER['REQUEST_URI'] == '' ) {
                $phpdetected = $_SERVER['REDIRECT_URL'];
            } else {
        ...
        function checkreferrer( $currurl = null, $referer = null )
        {
            ...
            if ( $this->ignore_current_url( $currurl ) ) {
                return;
            }
            // Sanitizing
            // $currurl = esc_url_raw($currurl); // results in error later on
            // $referer = esc_url_raw($referer); // results in error later on
                    ...
                    if ( $postid && $termlength > $dynamic_tag_minlength && $termlength < $dynamic_tag_maxlength ) {
                        ...
                        if ( $termcount < $dynamic_tag_max ) {
                            ...
                            if ( $term !== 0 && $term !== null ) {
                                ...
                                $this->log( "Created a new term <a href='" . admin_url( "edit-tags.php?action=edit&taxonomy={$dynamic_tag_tax}&tag_ID={$term_id}" ) . "'>{$gquery}</a> in the '{$dynamic_tag_tax}' taxonomy. For <a href='" . get_permalink( $postid ) . "' target='_blank'>" . get_permalink( $postid ) . "</a> (ID {$postid})", 10 );
                                // todo i8n
                                // todo - change to ->insert
                            ...
                            if ( is_wp_error( $setresult ) ) {
                                ...
                                $this->log( 'Error dynamic tagging - ' . $error_string );
                                // todo i8n
                            } else {
                                $this->log( sprintf(
        ...
        function do_seobooster_maintenance()
        {
            global  $wpdb ;
            $changecount = 0;
            $this->log( __( "Running daily maintenance routines.", 'seo-booster' ), 0 );
            ...
            $ownrefsquery = "SELECT id,ref,lp FROM {$bltable} WHERE ref LIKE '%{$site_url}%' OR ref LIKE '%{$site_url_no_http}%' limit 5000;";
            ...
            $query = "SELECT id,ref,lp,lastvisit FROM `{$bltable}` ORDER BY RAND() LIMIT 5000;";
                ...
                foreach ( $results as $res ) {
                    ...
                    if ( $searchref ) {
                        ...
                        if ( $this->ignorelink( $res['ref'] ) ) {
                            ...
                            // filter backlinks and unwanted links
                            if ( get_option( 'seobooster_debug_logging' ) == 'on' ) {
                                $this->log( sprintf( __( "Maintenance - Removing a reference to <code>%s</code>.", 'seo-booster' ), $res['ref'] ) );
                            }
                        }
